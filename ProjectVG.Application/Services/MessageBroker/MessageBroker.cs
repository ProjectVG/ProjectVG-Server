...
namespace ProjectVG.Application.Services.MessageBroker
{
    public class MessageBroker : IMessageBroker
    {
        ...
        public async Task<bool> SendResultAsync(string sessionId, byte[] audioData, string contentType, float? audioLength)
        {
            try
            {
                var session = await _clientSessionService.GetSessionAsync(sessionId);
                ...
                return await SendToEndpointAsync(session.ClientIP, session.ClientPort, audioData, contentType, audioLength);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "세션에 오디오 데이터 전송 중 오류: {SessionId}", sessionId);
                return false;
            }
        }
        ...
        public async Task<bool> SendToEndpointAsync(string ip, int port, byte[] audioData, string contentType, float? audioLength)
        {
            try
            {
                if (!IPAddress.TryParse(ip, out var ipAddress))
                ...
                // 오디오 데이터 전송
                var sentBytes = await _udpClient.SendAsync(audioData, audioData.Length, endpoint);
                
                var durationText = audioLength.HasValue ? $"{audioLength:F2}s" : "unknown";
                _logger.LogDebug("오디오 데이터 전송 완료: {IP}:{Port} -> {AudioLength} bytes, ContentType: {ContentType}, Duration: {Duration}", 
                    ip, port, sentBytes, contentType, durationText);
                return sentBytes > 0;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "오디오 데이터 전송 중 오류: {IP}:{Port}", ip, port);
                return false;
            }
        }
