...
if [ ! -f "deploy/docker-compose.yml" ]; then
    echo "ERROR: deploy/docker-compose.yml 파일이 존재하지 않습니다."
    echo "CI/CD에서 PROD_DOCKER_COMPOSE가 올바르게 설정되지 않았습니다."
    exit 1
fi
...
if [ $retry_count -eq $max_retries ]; then
    ...
    echo ""
    docker-compose -f deploy/docker-compose.yml logs --tail=50 projectvg-api
    exit 1
fi
